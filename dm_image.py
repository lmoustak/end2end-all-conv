...
def add_img_margins(img, margin_size):
    ...
    enlarged_img = np.zeros(
        (img.shape[0] + margin_size * 2, img.shape[1] + margin_size * 2))
...
class DMImgListIterator(Iterator):
    ...
    def __init__(self,
        ...
        super(DMImgListIterator, self).__init__(self.nb_sample, batch_size,
                                                shuffle, seed)
...
class DMExamListIterator(Iterator):
    ...
    def __init__(self,
        ...
        if verbose:
            print(
                'For left breasts, normal=%d, cancer=%d, unimaged/masked=%d.' %
                (np.sum(self.classes[:,
                                     0] == 0), np.sum(self.classes[:, 0] == 1),
                 np.sum(np.isnan(self.classes[:, 0]))))
            print(
                'For right breasts, normal=%d, cancer=%d, unimaged/masked=%d.'
                % (np.sum(self.classes[:, 1] == 0),
                   np.sum(self.classes[:, 1] == 1),
                   np.sum(np.isnan(self.classes[:, 1]))))

        super(DMExamListIterator, self).__init__(self.nb_exam, batch_size,
                                                 shuffle, seed)
    ...
    def next(self):
        with self.lock:
            index_array, current_index, current_batch_size = next(
                self.index_generator)
            classes_ = np.array([
                1 if p[0] or p[1] else 0 for p in self.classes[index_array, :]
            ])
            # Obtain the random state for the current batch.
            rng = RandomState() if self.seed is None else \
                RandomState(int(self.seed) + self.total_batches_seen)
            while self.all_neg_skip > rng.uniform() and np.all(classes_ == 0):
                index_array, current_index, current_batch_size = next(
                    self.index_generator)
                classes_ = np.array([
                    1 if p[0] or p[1] else 0
                    for p in self.classes[index_array, :]
                ])
                rng = RandomState() if self.seed is None else \
                    RandomState(int(self.seed) + self.total_batches_seen)
        if self.balance_classes:
        ...
        if self.prediction_mode:
            ...
            batch_x_cc = np.zeros((current_batch_size, ) + self.image_shape,
                                  dtype='float32')
            batch_x_mlo = np.zeros((current_batch_size, ) + self.image_shape,
                                   dtype='float32')
            ...
            try:
                if self.prediction_mode:
                    ...
                    if self.validation_mode:
                        fname = img_df['filename'].iloc[
                            0]  # read the canonical view.
                    else:  # training mode.
                        fname = img_df['filename'].sample(
                            1, random_state=rng).iloc[0]
                    img = read_resize_img(fname,
                                          self.target_size,
                                          target_scale=self.target_scale,
                                          gs_255=self.gs_255)
        ...
        def read_breast_imgs(breast_dat, **kwargs):
            ...
            for i, img_cc_ in enumerate(img_cc):
                # Always have one channel.
                if self.data_format == 'channels_first':
                    img_cc[i] = img_cc_.reshape(
                        (1, img_cc_.shape[0], img_cc_.shape[1]))
                else:
                    img_cc[i] = img_cc_.reshape(
                        (img_cc_.shape[0], img_cc_.shape[1], 1))
            for i, img_mlo_ in enumerate(img_mlo):
                if self.data_format == 'channels_first':
                    img_mlo[i] = img_mlo_.reshape(
                        (1, img_mlo_.shape[0], img_mlo_.shape[1]))
                else:
                    img_mlo[i] = img_mlo_.reshape(
                        (img_mlo_.shape[0], img_mlo_.shape[1], 1))
        ...
        if self.save_to_dir:
            ...
            for i in xrange(current_batch_size):
                if not self.prediction_mode:
                    ...
                    for ii, img_mlo_ in enumerate(batch_x_mlo[i]):
                        save_aug_img(img_mlo_, current_index * 2 + i, 'mlo',
                                     ii)

        # build batch of labels
        flat_classes = self.classes[
            index_array, :].ravel()  # [L, R, L, R, ...]
        # flat_classes = flat_classes[np.logical_not(np.isnan(flat_classes))]
        flat_classes[np.isnan(
            flat_classes)] = 0  # fill in non-cancerous labels.
        if self.class_mode == 'sparse':
...
class DMCandidROIIterator(Iterator):
    ...
    def __init__(self,
        ...
        super(DMCandidROIIterator, self).__init__(self.nb_sample,
                                                  img_per_batch, shuffle, seed)
    ...
    def next(self):
        with self.lock:
            ...
            if self.classes is not None:
                ...
                while self.all_neg_skip > rng.uniform() and np.all(
                        batch_cls == 0):
                    index_array, current_index, current_batch_size = \
                        next(self.index_generator)
                    batch_cls = self.classes[index_array]
        ...
        for ii, fi in enumerate(index_array):  # ii: image idx; fi: fname idx.
            img = read_resize_img(self.filenames[fi],
            ...
            # blob detection.
            key_pts = self.blob_detector.detect(
                (img / img.max() * 255).astype('uint8'))
        ...
        if self.roi_clf is None:
            ...
            with self.tf_graph.as_default():
                batch_s = self.roi_clf.predict(batch_x, batch_size=self.clf_bs)
        else:
            batch_s = self.roi_clf.predict(batch_x, batch_size=self.clf_bs)
...
class DMNumpyArrayIterator(Iterator):
    def __init__(self,
        ...
        if self.x.ndim != 4:
            raise ValueError(
                'Input data in `DMNumpyArrayIterator` '
        ...
        self.save_format = save_format
        self.seed = seed
        super(DMNumpyArrayIterator, self).__init__(x.shape[0], batch_size,
                                                   shuffle, seed)
    ...
    def next(self):
        ...
        with self.lock:
            ...
            if self.y is not None:
                ...
                # import pdb; pdb.set_trace()
                while self.no_pos_skip > rng.uniform() and np.all(
                        sparse_y != 1):
                    index_array, current_index, current_batch_size = \
                        next(self.index_generator)
                    batch_y = self.y[index_array]
                ...
                for fname in files:
                    ...
                    if is_valid:
                        ...
                        self.filenames.append(
                            path.relpath(absolute_path, directory))
        super(DMDirectoryIterator, self).__init__(self.nb_sample, batch_size,
                                                  shuffle, seed)
...
class DMImageDataGenerator(ImageDataGenerator):
    ...
    def flow_from_img_list(self,
                           ...
                           save_format='jpeg',
                           verbose=True):
        return DMImgListIterator(img_list,
                                 lab_list,
                                 self,
    ...
    def flow_from_exam_list(self,
                            ...
                            save_format='jpeg',
                            verbose=True):
        return DMExamListIterator(exam_list,
                                  self,
                                  target_size=target_size,
                                  target_scale=target_scale,
                                  gs_255=gs_255,
    ...
    def flow_from_candid_roi(self,
                             ...
                             save_format='jpeg',
                             verbose=True):
        return DMCandidROIIterator(self,
                                   img_list,
                                   lab_list,
    ...
    def flow_from_directory(self,
                            ...
                            save_format='jpeg',
                            follow_links=False):
        return DMDirectoryIterator(directory,
                                   self,
                                   target_size=target_size,
                                   target_scale=target_scale,
                                   gs_255=gs_255,

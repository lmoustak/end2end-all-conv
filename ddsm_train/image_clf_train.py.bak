...
def run(train_dir, val_dir, test_dir, patch_model_state=None, resume_from=None,
    ...
    if patch_net != 'yaroslav' or patch_net != 'custom':
        dup_3_channels = True
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--patch-model-state",
                        dest="patch_model_state", type=str, default=None)
    parser.add_argument("--no-patch-model-state",
                        dest="patch_model_state", action="store_const", const=None)
    parser.add_argument("--resume-from", dest="resume_from",
                        type=str, default=None)
    parser.add_argument("--no-resume-from", dest="resume_from",
                        action="store_const", const=None)
    parser.add_argument("--img-size", "-is", dest="img_size",
                        nargs=2, type=int, default=[1152, 896])
    parser.add_argument("--img-scale", "-ic",
                        dest="img_scale", type=float, default=None)
    parser.add_argument("--no-img-scale", "-nic",
                        dest="img_scale", action="store_const", const=None)
    parser.add_argument("--rescale-factor",
                        dest="rescale_factor", type=float, default=None)
    parser.add_argument("--no-rescale-factor",
                        dest="rescale_factor", action="store_const", const=None)
    parser.add_argument("--featurewise-center",
                        dest="featurewise_center", action="store_true")
    parser.add_argument("--no-featurewise-center",
                        dest="featurewise_center", action="store_false")
    parser.set_defaults(featurewise_center=True)
    parser.add_argument("--featurewise-mean",
                        dest="featurewise_mean", type=float, default=52.16)
    parser.add_argument("--equalize-hist",
                        dest="equalize_hist", action="store_true")
    parser.add_argument("--no-equalize-hist",
                        dest="equalize_hist", action="store_false")
    parser.set_defaults(equalize_hist=False)
    parser.add_argument("--batch-size", "-bs",
                        dest="batch_size", type=int, default=64)
    parser.add_argument("--train-bs-multiplier",
                        dest="train_bs_multiplier", type=float, default=.5)
    parser.add_argument(
        "--augmentation", dest="augmentation", action="store_true")
    parser.add_argument("--no-augmentation",
                        dest="augmentation", action="store_false")
    parser.set_defaults(augmentation=True)
    parser.add_argument("--class-list", dest="class_list", nargs='+', type=str,
                        default=['neg', 'pos'])
    parser.add_argument("--patch-net", dest="patch_net",
                        type=str, default="resnet50")
    parser.add_argument("--block-type", dest="block_type",
                        type=str, default="resnet")
    parser.add_argument("--top-depths", dest="top_depths",
                        nargs='+', type=int, default=[512, 512])
    ...
    parser.add_argument("--bottleneck-enlarge-factor", dest="bottleneck_enlarge_factor",
                        type=int, default=4)
    parser.add_argument("--add-heatmap", dest="add_heatmap",
                        action="store_true")
    parser.add_argument("--no-add-heatmap",
                        dest="add_heatmap", action="store_false")
    parser.set_defaults(add_heatmap=False)
    parser.add_argument("--avg-pool-size", dest="avg_pool_size",
                        nargs=2, type=int, default=[7, 7])
    ...
    parser.add_argument(
        "--add-shortcut", dest="add_shortcut", action="store_true")
    parser.add_argument("--no-add-shortcut",
                        dest="add_shortcut", action="store_false")
    parser.set_defaults(add_shortcut=False)
    parser.add_argument("--hm-strides", dest="hm_strides",
                        nargs=2, type=int, default=[1, 1])
    parser.add_argument("--hm-pool-size", dest="hm_pool_size",
                        nargs=2, type=int, default=[5, 5])
    parser.add_argument("--fc-init-units",
                        dest="fc_init_units", type=int, default=64)
    parser.add_argument("--fc-layers", dest="fc_layers", type=int, default=2)
    parser.add_argument(
        "--top-layer-nb", dest="top_layer_nb", type=int, default=None)
    parser.add_argument("--no-top-layer-nb", dest="top_layer_nb",
                        action="store_const", const=None)
    parser.add_argument("--nb-epoch", "-ne",
                        dest="nb_epoch", type=int, default=5)
    parser.add_argument("--all-layer-epochs",
                        dest="all_layer_epochs", type=int, default=20)
    parser.add_argument(
        "--load-val-ram", dest="load_val_ram", action="store_true")
    parser.add_argument("--no-load-val-ram",
                        dest="load_val_ram", action="store_false")
    parser.set_defaults(load_val_ram=False)
    parser.add_argument("--load-train-ram",
                        dest="load_train_ram", action="store_true")
    parser.add_argument("--no-load-train-ram",
                        dest="load_train_ram", action="store_false")
    parser.set_defaults(load_train_ram=False)
    parser.add_argument("--weight-decay", "-wd",
                        dest="weight_decay", type=float, default=.0001)
    parser.add_argument("--hidden-dropout", "-hd",
                        dest="hidden_dropout", type=float, default=.0)
    parser.add_argument("--weight-decay2", "-wd2",
                        dest="weight_decay2", type=float, default=.0001)
    parser.add_argument("--hidden-dropout2", "-hd2",
                        dest="hidden_dropout2", type=float, default=.0)
    parser.add_argument("--optimizer", dest="optim", type=str, default="sgd")
    parser.add_argument("--init-learningrate", "-ilr",
                        dest="init_lr", type=float, default=.01)
    parser.add_argument("--lr-patience", "-lrp",
                        dest="lr_patience", type=int, default=10)
    parser.add_argument("--es-patience", "-esp",
                        dest="es_patience", type=int, default=25)
    parser.add_argument("--auto-batch-balance",
                        dest="auto_batch_balance", action="store_true")
    parser.add_argument("--no-auto-batch-balance",
                        dest="auto_batch_balance", action="store_false")
    parser.set_defaults(auto_batch_balance=True)
    parser.add_argument("--pos-cls-weight",
                        dest="pos_cls_weight", type=float, default=1.0)
    parser.add_argument("--neg-cls-weight",
                        dest="neg_cls_weight", type=float, default=1.0)
    parser.add_argument("--all-layer-multiplier",
                        dest="all_layer_multiplier", type=float, default=.1)
    ...
    if args.patch_model_state is None and args.resume_from is None:
        raise Exception(
            'One of [patch_model_state, resume_from] must not be None.')
    run_opts = dict(
